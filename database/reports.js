...
const countPending = async () => (await get('SELECT COUNT(*) as c FROM reports WHERE status=$1',['pending']))?.c||0;

const resolveReport = id => run("UPDATE reports SET status='resolved' WHERE id=$1",[id]);
const dismissReport = id => run("UPDATE reports SET status='dismissed' WHERE id=$1",[id]);

const hasReported = async (userId,fileId) => {
  const key='rep_'+userId+'_'+fileId;
  const c=cacheGet(key);
  if(c!==null) return c;
  const r=!!(await get("SELECT 1 FROM reports WHERE user_id=$1 AND file_id=? AND status='pending'",[userId,fileId]));
