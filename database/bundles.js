...
const getBundleFiles = bundleId => all('SELECT bf.*, f.title as file_title, f.file_type as real_type, f.is_deleted FROM bundle_files bf LEFT JOIN files f ON CAST(bf.file_id AS BIGINT) = f.id WHERE bf.bundle_id=? AND (f.is_deleted=0 OR f.is_deleted IS NULL)',[bundleId]);
