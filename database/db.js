...
function scheduleSave() {
  ...
  saveTimer = setTimeout(() => {
    ...
    dirty = false;
    try { 
      fs.writeFile(DB_PATH, Buffer.from(sqlJs.export()), (err) => {
...
function toPgCached(sql) {
  if(pgSqlCache.has(sql)) {
    ...
    pgSqlCache.delete(sql);
    pgSqlCache.set(sql, val);
    return val;
  }
  ...
  cacheSize++;
  if(cacheSize > 500) {
    const keys = Array.from(pgSqlCache.keys());
    pgSqlCache.delete(keys[0]);
    cacheSize--;
  }
  return converted;
}
...
async function initSchema() {
  ...
  const tables = [
    `DROP VIEW IF EXISTS vw_active_today;
