...
const getFile = async id => { const k='file_'+id; const c=cacheGet(k); if(c) return c; const r=await get(J+' WHERE f.id=$1 AND f.is_deleted=0',[id]); if(r) cacheSet(k,r,600000); return r; };
const getFiles = async catId => { const k='files_cat_'+catId; const c=cacheGet(k); if(c) return c; const r=await all(J+' WHERE f.category_id=$1 AND f.is_deleted=0 ORDER BY f.uploaded_at DESC',[catId]); cacheSet(k,r,600000); return r; };

const invalidateFilesCache = catId => { cacheClearPrefix('files_cat_'+catId); cacheClearPrefix('showfiles_'+catId); };
...
const incDownloads = async id => { await run('UPDATE files SET downloads=downloads+1 WHERE id=$1',[id]); cacheClear('file_'+id); cacheClear('prev_static_'+id); };
const softDelete = async id => { if(global._clearSearchCache) global._clearSearchCache(); const f=await get('SELECT category_id FROM files WHERE id=$1',[id]); await run('UPDATE files SET is_deleted=1 WHERE id=$1',[id]); cacheClear('file_'+id); cacheClear('prev_static_'+id); cacheClear('similar_'+id); if(f) invalidateFilesCache(f.category_id); };
const restore = async id => { const f=await get('SELECT category_id FROM files WHERE id=$1',[id]); await run('UPDATE files SET is_deleted=0 WHERE id=$1',[id]); cacheClear('file_'+id); if(f) invalidateFilesCache(f.category_id); };
const rename = async (id,title) => { const f=await get('SELECT category_id FROM files WHERE id=$1',[id]); await run('UPDATE files SET title=$1 WHERE id=$2',[title,id]); cacheClear('file_'+id); cacheClear('prev_static_'+id); if(f) invalidateFilesCache(f.category_id); };
const updateDesc = async (id,desc) => { await run('UPDATE files SET description=$1 WHERE id=$2',[desc,id]); cacheClear('file_'+id); cacheClear('prev_static_'+id); };
const updateDescMd = updateDesc;

const search = (q, limit = 20) => {
  const w = '%' + q + '%';
  return all(
    J + ` JOIN semesters sm ON s.semester_id=sm.id JOIN years y ON sm.year_id=y.id JOIN specialties sp ON y.specialty_id=sp.id
     WHERE f.is_deleted=0 AND (f.title ILIKE $1 OR s.name ILIKE $1 OR c.name ILIKE $1 OR f.description ILIKE $1)
     ORDER BY f.downloads DESC LIMIT $2`,
    [w, limit]
  );
};
