...
// query خفيف لقائمة الملفات
const J_LIGHT = `SELECT f.id,f.title,f.file_type,f.downloads,f.is_deleted FROM files f`;
...
const softDelete = id => { cacheClear('file_'+id); return run('UPDATE files SET is_deleted=1 WHERE id=?', [id]); };
const restore = id => { cacheClear('file_'+id); return run('UPDATE files SET is_deleted=0 WHERE id=?', [id]); };
const rename = (id,title) => { cacheClear('file_'+id); return run('UPDATE files SET title=? WHERE id=?', [title,id]); };
const updateDesc = (id,desc) => { cacheClear('file_'+id); return run('UPDATE files SET description=? WHERE id=?', [desc,id]); };
const incDownloads = id => { cacheClear('file_'+id); return run('UPDATE files SET downloads=downloads+1 WHERE id=?', [id]); };
