...
const getPending = () => all("SELECT sm.*,mt.name,mt.type,mt.content,mt.file_id FROM scheduled_messages sm LEFT JOIN message_templates mt ON sm.template_id=mt.id WHERE sm.sent=0 AND sm.send_at::timestamp <= NOW() + INTERVAL '1 hour'");
