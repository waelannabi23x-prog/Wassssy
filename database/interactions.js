...
const addHistory = (uid,fid) => {
  ...
  if(_histDedup.size>5000) { var old=now-600000; for(const [kk,v] of _histDedup) if(v<old) _histDedup.delete(kk); }
  return run('INSERT INTO history(user_id,file_id) VALUES($1,$2)',[uid,fid]);
};

const getHistory = (uid,n) => { n=n||15; return all(J+' JOIN history h ON h.file_id=f.id WHERE h.user_id=$1 AND f.is_deleted=0 ORDER BY h.viewed_at DESC LIMIT $2',[uid,n]); };
...
const getUserDownloadCount = async uid => {
  ...
  var r=(await get('SELECT COUNT(*) as c FROM history WHERE user_id=$1',[uid])); var cnt=r?r.c:0;
...
const addLog = (uid,action,details) => run('INSERT INTO logs(user_id,action,details) VALUES($1,$2,$3)',[uid,action,details||'']).catch(function(){});

const getLogs = (n) => { n=n||20; return all('SELECT l.*,u.first_name FROM logs l LEFT JOIN users u ON l.user_id=u.id ORDER BY l.created_at DESC LIMIT $1',[n]); };
...
const getActiveUsers = async (days) => {
  ...
  var ids=rows.map(function(r){return r.id;}); cacheSet(k,ids,600000); return ids;
};
