...
const addFav = (uid,fid) => { cacheClear('fav_'+uid+'_'+fid); cacheClear('favcnt_'+fid); return run('INSERT INTO favorites(user_id,file_id) VALUES(?,?) ON CONFLICT(user_id,file_id) DO NOTHING',[uid,fid]); };
const removeFav = (uid,fid) => { cacheClear('fav_'+uid+'_'+fid); cacheClear('favcnt_'+fid); return run('DELETE FROM favorites WHERE user_id=? AND file_id=?',[uid,fid]); };
...
async function getRecommended(uid,limit=8) {
  const cats=(await all(`SELECT c.id FROM history h JOIN files f ON h.file_id=f.id JOIN categories c ON f.category_id=c.id WHERE h.user_id=? GROUP BY c.id ORDER BY MAX(h.viewed_at) DESC LIMIT 5`,[uid])).map(r=>r.id);
  if(!cats.length) return all(J+' WHERE f.is_deleted=0 ORDER BY f.downloads DESC LIMIT ?',[limit]);
  const ph=cats.map(()=>'?').join(',');
  return all(J+` WHERE f.category_id IN (${ph}) AND f.is_deleted=0 AND f.id NOT IN (SELECT file_id FROM history WHERE user_id=?) ORDER BY f.downloads DESC LIMIT ?`,[...cats,uid,limit]);
}
...
module.exports = { getFavs,isFav,addFav,removeFav,favCount,addHistory,getHistory,getLastFile,getUserDownloadCount,getRecommended,getSimilar,addLog,getLogs,getActiveUsers,addRating,getUserRating,getAvgRating,getFavBatch,getRatingBatch };
