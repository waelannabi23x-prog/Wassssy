...
async function handleFlip(ctx) {
  ...
  if(!bet||bet<100) return ctx.reply('🎲 *قلب العملة*\n\nالاستخدام: /flip [مبلغ]\nمثال: /flip 500\nالحد الأدنى: 100$',{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
  const acc=await ensureAcc(ctx);
  if((acc.balance||0)<bet) return ctx.reply('❌ رصيدك غير كافٍ!\n💰 رصيدك: *'+fmt(acc.balance)+'*',{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
  const maxBet=Math.min(Math.floor((acc.balance||0)*0.5),50000);
  if(bet>maxBet) return ctx.reply('⚠️ الحد الأقصى: *'+fmt(maxBet)+'*',{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
  ...
  if(wm) ctx.telegram.editMessageText(ctx.chat.id,wm.message_id,null,txt,{parse_mode:'Markdown'}).catch(()=>ctx.reply(txt,{parse_mode:'Markdown'}).catch(()=>{}));
  else ctx.reply(txt,{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
}

async function handleRob(ctx) {
  const uid=ctx.from?.id;
  const target=ctx.message?.reply_to_message?.from;
  if(!target||target.is_bot||target.id===uid) return ctx.reply('🦹 *السرقة*\n\nرد على رسالة شخص لتسرقه!\nنسبة نجاح: 40% — عند الفشل: غرامة 5%',{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
  const rk='rob_'+uid;
  ...
  if(ok2){
    ...
    return ctx.reply('🦹 *السرقة نجحت!*\n\n🎯 الضحية: *'+(target.first_name||'مجهول')+'*\n💰 المسروق: *'+fmt(s)+'*\n🏦 رصيدك: *'+fmt((ra.balance||0)+s)+'*',{parse_mode:'Markdown',reply_to_message_id:ctx.message?.message_id}).catch(()=>{});
  } else {
