...
const PS = 10;
const setState = (uid, s) => global.setState?.(uid, s);
const clearState = uid => global.delState?.(uid);

const ALL_PERMS = ['upload', 'delete', 'add_content', 'view_users', 'full'];
const PERM_LABELS = {
  upload: '📤 رفع', delete: '🗑 حذف', add_content: '➕ إضافة محتوى',
  view_users: '👥 مشاهدة المستخدمين', full: '👑 كل الصلاحيات'
};
...
async function concurrentBroadcast(bot, chatId, msgId, ids, txt, opt = {}) {
  ...
  const updateUI = async () => {
    ...
    bot.telegram.editMessageText(chatId, msgId, null, 
      `📢 *جاري الإرسال...*\n\`[${bar}] ${progress}%\`\n✅ ${sent} | ❌ ${failed} | ⏳ ${total - sent - failed}`, 
      { parse_mode: 'Markdown' }
    ).catch(() => {});
  };
  for (let i = 0; i < total; i += 30) {
    const results = await Promise.allSettled(
      ids.slice(i, i + 30).map(id => bot.telegram.sendMessage(id, txt, opt).then(() => true).catch(() => false))
    );
    results.forEach(x => { if (x.status === 'fulfilled' && x.value) sent++; else failed++; });
    await updateUI();
    if (i + 30 < total) await new Promise(r => setTimeout(r, 50));
  }
  return { sent, failed };
}

async function mainMenu(ctx) {
  const [specs, files] = await Promise.all([content.getSpecs(), filesDb.totalFiles()]);
  const text = `🛠 *لوحة الإدارة*\n\n📚 التخصصات: *${specs.length}*\n📁 الملفات: *${files}*\n🔧 الصيانة: *${global.maintenanceMode ? '🔴 مفعّل' : '🟢 متوقف'}*`;
  const rows = [
    [btn('📂 المحتوى', 'mg_content')],
    [btn('📊 الإحصائيات', 'mg_analytics'), btn('📜 السجلات', 'mg_logs')]
  ];
  if (isOwner(ctx.uid)) {
    rows.push(
      [btn('📢 بث', 'mg_broadcast'), btn('👥 المستخدمون', 'mg_users')],
      [btn('👑 الإداريون', 'mg_admins')],
      [btn('💾 نسخ احتياطي', 'mg_backup'), btn(global.maintenanceMode ? '🟢 إيقاف الصيانة' : '🔴 وضع الصيانة', 'mg_maint')],
      [btn('♻️ استعادة', 'mg_restore'), btn('🗑 سلة المحذوفات', 'mg_trash')],
      [btn('🔔 إشعار للمستخدمين', 'mg_notify'), btn('📣 إشعار القروبات', 'mg_notify_groups')],
      [btn('🚩 البلاغات', 'mg_reports'), btn('📨 نظام الرسائل', 'mg_msgs')],
      [btn('🎓 إشعار لتخصص', 'mg_notify_sp')]
    );
  }
...
async function showMgFiles(ctx, spId, yrId, smId, sbId, catId, page = 0) {
  const [cat, allFiles] = await Promise.all([content.getCategory(catId), filesDb.getFiles(catId)]);
  ...
  list.forEach(f => {
    ...
    rows.push([
      btn('✏️', `mg_rn_fl_${spId}_${yrId}_${smId}_${sbId}_${catId}_${f.id}`),
      btn('📝', `mg_desc_fl_${spId}_${yrId}_${smId}_${sbId}_${catId}_${f.id}`),
      btn('🗑', `mg_dl_fl_${spId}_${yrId}_${smId}_${sbId}_${catId}_${f.id}`)
    ]);
  });
  if (total > PS) {
    const nav = [];
  ...
  if (bundles.length) {
    ...
    bundles.forEach(b => rows.splice(1, 0, [btn(`📦 ${b.title}`, `bundle_${b.id}_${spId}_${yrId}_${smId}_${sbId}_${catId}`)]));
  }
...
async function showAnalytics(ctx) {
  ...
  const [top, recent, totalUsers, activeToday, totalFiles, totalDl, specs, totalGroups, topGroups] = await Promise.all([
    filesDb.topDownloaded(5), filesDb.recentFiles(5), usersDb.count(), usersDb.activeToday(),
    filesDb.totalFiles(), filesDb.totalDownloads(), content.getSpecs(),
  ...
  const [spDist, topUsers, peakHours, topCats] = await Promise.all([
    ...
    all(`SELECT EXTRACT(HOUR FROM viewed_at) as hour,COUNT(*) as cnt FROM history GROUP BY hour ORDER BY cnt DESC LIMIT 3`),
  ...
  if (topGroups?.length) topGroups.forEach((g, i) => { text += `${i + 1}. ${escMd(g.title || 'بدون اسم')} (${escMd(g.sp_name || '?')}) — *${g.members}*\n`; });
...
async function showUsers(ctx, page = 0) {
  ...
  const rows = list.map(u => [btn(`👤 ${u.first_name || u.id}`, `mg_profile_${u.id}`), btn(u.is_banned ? '✅' : '🚫', `${u.is_banned ? 'mg_unban_' : 'mg_ban_'}${u.id}`)]);
  const nav = [];
...
async function showUserProfile(ctx, userId) {
  const [user, dlCount, favCount, spRow, lastFile] = await Promise.all([
    usersDb.getById(userId), interactions.getUserDownloadCount(userId),
    require('../database/db').get('SELECT COUNT(*) as c FROM favorites WHERE user_id=$1', [userId]).then(r => r?.c || 0),
    usersDb.getSpecialty(userId), interactions.getLastFile(userId)
  ]);
  ...
  const rows = [[btn(user.is_banned ? '✅ إلغاء الحظر' : '🚫 حظر', `${user.is_banned ? 'mg_unban_' : 'mg_ban_'}${userId}`)], [back('mg_users')[0]]];
  return eos(ctx, text, { parse_mode: 'Markdown', ...build(rows) });
}
...
async function showMsgsMenu(ctx) {
  const [templates, scheduled] = await Promise.all([messagesDb.getTemplates(), messagesDb.getScheduled()]);
  const text = `📨 *نظام الرسائل*\n\n📝 القوالب: *${templates.length}*\n📅 المجدولة: *${scheduled.length}*`;
  const rows = [[btn('📝 القوالب', 'mg_templates'), btn('📅 المجدولة', 'mg_scheduled')], [btn('➕ قالب جديد', 'mg_add_template')], back('mg_menu')];
  return eos(ctx, text, { parse_mode: 'Markdown', ...build(rows) });
}
...
async function handleFileUpload(ctx) {
  ...
  else if (msg.audio || msg.voice) { fid = (msg.audio || msg.voice).file_id; ftype = 'document'; }
  ...
  try {
    ...
    ctx.reply(`✅ *${escMd(state.title)}* رُفع بنجاح!`, {
      parse_mode: 'Markdown', ...build([
        [btn('➕ رفع آخر', `mg_upl_${state.spId}_${state.yrId}_${state.smId}_${state.sbId}_${state.catId}`)],
        [btn('📁 عرض الملفات', `mg_fls_${state.spId}_${state.yrId}_${state.smId}_${state.sbId}_${state.catId}`)]
      ])
    });
  } catch (e) {
    clearState(uid);
    ctx.reply(e.message === 'exists' ? '❌ يوجد ملف بهذا الاسم!' : '❌ فشل: ' + e.message);
  }
}
  ...
  try {
    switch (state.type) {
      ...
      case 'mg_admin_search': {
        ...
        ctx.reply(resp, { parse_mode: 'Markdown', ...build([back('mg_menu')]) });
        break;
      }
      case 'mg_broadcast': {
        clearState(uid);
        const ids = await usersDb.allIds();
        const sm = await ctx.reply(`📢 *جاري الإرسال...*\n\`[░░░░░░░░░░] 0%\`\n✅ 0 | ❌ 0 | ⏳ ${ids.length}`, { parse_mode: 'Markdown' });
        const bcRes = await concurrentBroadcast(ctx.telegram, ctx.chat.id, sm.message_id, ids, `📢 *إعلان*\n\n${text}`, { parse_mode: 'Markdown' });
        ctx.telegram.editMessageText(ctx.chat.id, sm.message_id, null, `✅ *اكتمل!*\n\`[██████████] 100%\`\n✅ ${bcRes.sent} | ❌ ${bcRes.failed}`, { ...build([back('mg_menu')]), parse_mode: 'Markdown' }).catch(() => {});
        break;
      }
      case 'mg_notify_sp_msg': {
        clearState(uid);
        const spUsers = await usersDb.getUsersBySpecialty(state.spId);
        const results = await Promise.allSettled(spUsers.map(id => ctx.telegram.sendMessage(id, `🔔 ${text}`, { parse_mode: 'Markdown' }).then(() => true).catch(() => false)));
        const spSent = results.filter(r => r.status === 'fulfilled' && r.value).length;
        ctx.reply(`✅ أُرسل لـ *${spSent}* مستخدم`, { parse_mode: 'Markdown', ...build([back('mg_menu')]) });
        break;
      }
      case 'mg_notify_groups_msg': {
        ...
        let gSent = 0, gFail = 0;
        for (const g of groups) { try { await ctx.telegram.sendMessage(g.chat_id, `📣 *إشعار*\n\n${text}`, { parse_mode: 'Markdown' }); gSent++; } catch (_) { gFail++; } await new Promise(r => setTimeout(r, 100)); }
        ctx.reply(`✅ أُرسل لـ *${gSent}* قروب${gFail ? ' | ❌ ' + gFail : ''}`, { parse_mode: 'Markdown', ...build([back('mg_menu')]) });
        break;
      }
      case 'mg_notify_msg': {
        clearState(uid);
        const nIds = await interactions.getActiveUsers(7);
        const results = await Promise.allSettled(nIds.map(id => ctx.telegram.sendMessage(id, `🔔 *إشعار*\n\n${text}`, { parse_mode: 'Markdown' }).then(() => true).catch(() => false)));
        const nSent = results.filter(r => r.status === 'fulfilled' && r.value).length;
        ctx.reply(`✅ أُرسل لـ *${nSent}* مستخدم نشط!`, { parse_mode: 'Markdown', ...build([back('mg_menu')]) });
        break;
      }
      case 'mg_add_admin_id': {
        ...
        clearState(uid);
        ctx.reply('اختر تخصص المشرف:', { ...build(spRows) });
        try { ctx.telegram.sendMessage(tid, '🎉 تمت إضافتك مشرفاً', { parse_mode: 'Markdown' }); } catch (_) {}
        break;
      }
      ...
      case 'mg_tpl_content': {
        ...
        } catch (e) { clearState(uid); ctx.reply(e.message === 'exists' ? '❌ قالب موجود!' : '❌ ' + e.message); }
        break;
      }
      case 'mg_sched_time': {
        try { await messagesDb.addScheduled(state.tplId, state.target, state.spId || 0, text); clearState(uid); ctx.reply('✅ تمت الجدولة!', build([[btn('📅 المجدولة', 'mg_scheduled')]])); }
        catch (e) { clearState(uid); ctx.reply('❌ ' + e.message); }
        break;
      }
      default: break;
    }
    ...
    clearState(uid);
    ctx.reply(e.message === 'exists' ? '❌ موجود!' : '❌ ' + e.message);
  }
}
...
async function handleCallback(ctx, data) {
  ...
  if (data === 'mg_users') {
    var p = ctx.isOwner ? ['full'] : await adminsDb.getPerms(ctx.uid);
    if (!p.includes('full') && !p.includes('view_users')) return ctx.answerCbQuery('ليس لديك صلاحية', { show_alert: true });
    return showUsers(ctx);
  }
  ...
  if (data === 'mg_notify_sp') {
    ...
    var rows = specs.map(function(s) { return [btn('🎓 ' + s.name, 'mg_notify_sp_' + s.id)]; });
    rows.push(back('mg_menu'));
    return eos(ctx, '🎓 اختر تخصص لإرسال الإشعار:', { parse_mode: 'Markdown', ...build(rows) });
  }
  if (data === 'mg_notify_groups') {
    var specs = await content.getSpecs();
    var rows = specs.map(function(s) { return [btn('🎓 ' + s.name, 'mg_ng_sp_' + s.id)]; });
    rows.push([btn('📣 كل القروبات', 'mg_ng_sp_0')], [btn('◀️ رجوع', 'mg_menu')]);
    return ctx.reply('📣 إشعار القروبات\n\nاختر التخصص:', { ...build(rows) }).catch(function(e) { ctx.reply('❌ ' + e.message); });
  }
  if (data.startsWith('mg_ng_sp_')) {
    var spId = data.replace('mg_ng_sp_', '');
    setState(uid, { type: 'mg_notify_groups_msg', spId: spId });
    return ctx.reply('📝 رسالة الإشعار لـ ' + (spId === '0' ? 'كل القروبات' : 'التخصص') + ':\n_(أو /cancel)_', { parse_mode: 'Markdown' });
  }
  if (data.startsWith('mg_notify_sp_') && !data.startsWith('mg_notify_sp_msg')) {
    var spId = data.replace('mg_notify_sp_', '');
    setState(uid, { type: 'mg_notify_sp_msg', spId: spId });
    return ctx.reply('📝 رسالة الإشعار:\n_(أو /cancel)_', { parse_mode: 'Markdown' });
  }
  ...
  if (data.startsWith('mg_tpl_') && !data.startsWith('mg_tpl_content')) {
    var id = data.replace('mg_tpl_', '');
    var t = await messagesDb.getTemplate(id);
    if (!t) return ctx.reply('❌ غير موجود');
    var icon = { text: '📝', photo: '🖼', document: '📄', link: '🔗', video: '🎥' }[t.type] || '📝';
    var rows = [[btn('📤 إرسال الآن', 'mg_send_now_' + id)], [btn('📅 جدولة', 'mg_sched_' + id)], [btn('🗑 حذف', 'mg_del_tpl_' + id)], [back('mg_templates')[0]]];
    return eos(ctx, icon + ' *' + escMd(t.name) + '*\nالنوع: ' + t.type + '\n\n' + escMd((t.content || '').substring(0, 200)), { parse_mode: 'Markdown', ...build(rows) });
  }
  if (data.startsWith('mg_del_tpl_')) { await messagesDb.deleteTemplate(data.replace('mg_del_tpl_', '')); return showTemplates(ctx); }
  
  if (data.startsWith('mg_sched_') && !data.startsWith('mg_sched_all_') && !data.startsWith('mg_sched_sp_')) {
    var tplId = data.replace('mg_sched_', '');
    var rows = [[btn('👥 كل المستخدمين', 'mg_sched_all_' + tplId)], [btn('🎓 تخصص معين', 'mg_sched_sp_' + tplId)], [back('mg_templates')[0]]];
    return eos(ctx, '📅 من تريد الإرسال؟', { parse_mode: 'Markdown', ...build(rows) });
  }
  ...
  if (data.startsWith('mg_sched_sp_') && !data.startsWith('mg_sched_spid_')) {
    var tplId = data.replace('mg_sched_sp_', '');
    var specs = await content.getSpecs();
    var rows = specs.map(function(s) { return [btn('🎓 ' + s.name, 'mg_sched_spid_' + tplId + '_' + s.id)]; });
    return eos(ctx, 'اختر التخصص:', { ...build(rows) });
  }
  if (data.startsWith('mg_sched_spid_')) {
    var p = data.replace('mg_sched_spid_', '').split('_');
    setState(uid, { type: 'mg_sched_time', tplId: p[0], target: 'specialty', spId: p[1] });
    return ctx.reply('📅 وقت الإرسال\nمثال: 2026-04-10 20:00');
  }
  
  if (data.startsWith('mg_send_now_')) {
    var tplId = data.replace('mg_send_now_', '');
    var tpl = await messagesDb.getTemplate(tplId);
    if (!tpl) return ctx.reply('❌ غير موجود');
    var ids = await usersDb.allIds();
    var sent = 0, failed = 0;
    var total = ids.length;
    var sm = await ctx.reply('📤 *جاري...*\n`[░░░░░░░░░░] 0%`\n✅ 0 | ❌ 0 | ⏳ ' + total, { parse_mode: 'Markdown' });
    var sendMsg = async function(id) {
      var o = { parse_mode: 'Markdown' };
      if (tpl.type === 'text') return ctx.telegram.sendMessage(id, tpl.content, o).then(function() { return 1; }).catch(function() { return 0; });
      if (tpl.type === 'photo') return ctx.telegram.sendPhoto(id, tpl.file_id, { caption: tpl.content, ...o }).then(function() { return 1; }).catch(function() { return 0; });
      if (tpl.type === 'document') return ctx.telegram.sendDocument(id, tpl.file_id, { caption: tpl.content, ...o }).then(function() { return 1; }).catch(function() { return 0; });
      if (tpl.type === 'video') return ctx.telegram.sendVideo(id, tpl.file_id, { caption: tpl.content, ...o }).then(function() { return 1; }).catch(function() { return 0; });
      if (tpl.type === 'link') return ctx.telegram.sendMessage(id, tpl.content).then(function() { return 1; }).catch(function() { return 0; });
      return 0;
    };
    for (var i = 0; i < ids.length; i += 30) {
      var r = await Promise.allSettled(ids.slice(i, i + 30).map(sendMsg));
      r.forEach(function(x) { if (x.status === 'fulfilled' && x.value) sent++; else failed++; });
      var p = Math.round((sent + failed) / total * 100);
      var b = '█'.repeat(Math.round(p / 10)) + '░'.repeat(10 - Math.round(p / 10));
      ctx.telegram.editMessageText(ctx.chat.id, sm.message_id, null, '📤 *جاري...*\n`[' + b + '] ' + p + '%`\n✅ ' + sent + ' | ❌ ' + failed + ' | ⏳ ' + (total - sent - failed), { parse_mode: 'Markdown' }).catch(function() {});
      if (i + 30 < total) await new Promise(function(r) { setTimeout(r, 50); });
    }
    return ctx.telegram.editMessageText(ctx.chat.id, sm.message_id, null, '✅ *اكتمل!*\n`[██████████] 100%`\n✅ ' + sent + ' | ❌ ' + failed, { parse_mode: 'Markdown', ...build([back('mg_templates')]) }).catch(function() {});
  }
  
  if (data.startsWith('mg_del_sched_')) { await messagesDb.deleteScheduled(data.replace('mg_del_sched_', '')); return showScheduled(ctx); }
  
  if (data === 'mg_reports') {
    var rpts = await all("SELECT r.*,f.title as ft,u.first_name as fn FROM reports r LEFT JOIN files f ON r.file_id=f.id LEFT JOIN users u ON r.user_id=u.id WHERE r.status='pending' ORDER BY r.created_at DESC LIMIT 20");
    var txt = '🚩 *البلاغات (' + rpts.length + ')*\n\n';
    if (!rpts.length) txt += 'لا توجد بلاغات.';
    else rpts.forEach(function(r, i) { txt += (i + 1) + '. ' + escMd(r.ft || '?') + ' | ' + escMd(r.reason || '?') + ' | ' + (r.fn || r.user_id) + '\n'; });
    var rrows = rpts.map(function(r) { return [btn('حذف', 'mg_cdl_fl_0_0_0_0_' + r.file_id), btn('تجاهل', 'mg_dismiss_report_' + r.id)]; });
    rrows.push(back('mg_menu'));
    return eos(ctx, txt, { parse_mode: 'Markdown', ...build(rrows) });
  }
  if (data.startsWith('mg_dismiss_report_')) {
    var rid = data.replace('mg_dismiss_report_', '');
    dbRun("UPDATE reports SET status='dismissed' WHERE id=$1", [rid]).catch(function() {});
  ...
  if (data === 'mg_maint') {
    ...
    var mText = '🔧 *الصيانة: ' + (global.maintenanceMode ? '🔴 مفعّلة' : '🟢 متوقفة') + '*';
    var mRows = [[btn(global.maintenanceMode ? '🟢 إيقاف' : '🔴 تفعيل', 'mg_maint')], [btn('📝 تعديل الرسالة', 'mg_set_maint_msg'), btn('◀️ رجوع', 'mg_menu')]];
    return eos(ctx, mText, { parse_mode: 'Markdown', ...build(mRows) });
  }
  ...
  if (data.startsWith('mg_admin_sp_')) {
    var p = data.replace('mg_admin_sp_', '').split('_');
    await adminsDb.setSpecialty(p[0], p[1]);
    return eos(ctx, '✅ تم تحديد التخصص', { ...build([back('mg_admins')]) });
  }
  if (data.startsWith('mg_da_')) {
    var rid = parseInt(data.replace('mg_da_', ''));
    await adminsDb.remove(rid);
    if (global.invalidateAdmin) global.invalidateAdmin(rid);
    return showAdmins(ctx);
  }
  if (data.startsWith('mg_ep_')) return showEditPerms(ctx, data.replace('mg_ep_', ''));
  if (data.startsWith('mg_tp_')) {
    var p = data.replace('mg_tp_', '').split('_');
    var adminId = p[0];
    var perm = p.slice(1).join('_');
    var list = await adminsDb.getAll();
    var admin = list.find(function(a) { return a.user_id == adminId; });
    var perms = (admin.permissions || '').split(',').map(function(x) { return x.trim(); }).filter(Boolean);
    if (perms.includes(perm)) perms = perms.filter(function(x) { return x !== perm; });
    else {
      if (perm === 'full') perms = ['full'];
      else { perms = perms.filter(function(x) { return x !== 'full'; }); perms.push(perm); }
    }
    await adminsDb.updatePerms(adminId, perms.join(','));
    return showEditPerms(ctx, adminId);
  }
  if (data.startsWith('mg_profile_')) return showUserProfile(ctx, data.replace('mg_profile_', ''));
  if (data.startsWith('mg_ban_')) {
    var bid = parseInt(data.replace('mg_ban_', ''));
    await usersDb.ban(bid); cacheClearPrefix('admin_users_'); cacheClear('ban_' + bid);
    await interactions.addLog(uid, 'ban', String(bid));
    return showUsers(ctx);
  }
  if (data.startsWith('mg_unban_')) {
    var ubid = parseInt(data.replace('mg_unban_', ''));
    await usersDb.unban(ubid); cacheClearPrefix('admin_users_'); cacheClear('ban_' + ubid);
    return showUsers(ctx);
  }
  ...
  if (data === 'mg_empty_trash') { var trashRows = [[btn('✅ تأكيد', 'mg_confirm_empty')], [btn('❌ إلغاء', 'mg_trash')]]; return eos(ctx, '⚠️ حذف الكل نهائياً؟', build(trashRows)); }
  ...
  if (data.startsWith('mg_sems_')) { var p = data.replace('mg_sems_', '').split('_'); return showSemesters(ctx, p[0], p[1]); }
  if (data.startsWith('mg_sbs_')) { var p = data.replace('mg_sbs_', '').split('_'); return showSubjects(ctx, p[0], p[1], p[2]); }
  if (data.startsWith('mg_cats_')) { var p = data.replace('mg_cats_', '').split('_'); return showCategories(ctx, p[0], p[1], p[2], p[3]); }
  if (data.startsWith('mg_fls_pg_')) { var p = data.replace('mg_fls_pg_', '').split('_'); return showMgFiles(ctx, p[0], p[1], p[2], p[3], p[4], parseInt(p[5])); }
  if (data.startsWith('mg_fls_')) { var p = data.replace('mg_fls_', '').split('_'); return showMgFiles(ctx, p[0], p[1], p[2], p[3], p[4]); }
  ...
  if (data.startsWith('mg_dl_sp_')) {
    var id = data.replace('mg_dl_sp_', '');
    var sp = await content.getSpec(id);
    var delRows = [[btn('✅ تأكيد', 'mg_cdl_sp_' + id), btn('❌ إلغاء', 'mg_content')]];
    return eos(ctx, '🗑 حذف *' + escMd(sp?.name || '') + '*?\n⚠️ سيتم حذف كل المحتوى!', { parse_mode: 'Markdown', ...build(delRows) });
  }
  ...
  if (data.startsWith('mg_rn_yr_')) { var p = data.replace('mg_rn_yr_', '').split('_'); setState(uid, { type: 'mg_rn_yr', id: p[1], spId: p[0] }); return ctx.reply('✏️ الاسم الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  
  if (data.startsWith('mg_dl_yr_')) {
    var p = data.replace('mg_dl_yr_', '').split('_');
    var yr = await content.getYear(p[1]);
    var delRows = [[btn('✅ نعم', 'mg_cdl_yr_' + p[0] + '_' + p[1]), btn('❌ لا', 'mg_yrs_' + p[0])]];
    return eos(ctx, '🗑 حذف *' + escMd(yr?.name || '') + '*?', { parse_mode: 'Markdown', ...build(delRows) });
  }
  if (data.startsWith('mg_cdl_yr_')) { var p = data.replace('mg_cdl_yr_', '').split('_'); await content.deleteYear(p[1]); return showYears(ctx, p[0]); }
  
  if (data.startsWith('mg_add_sem_')) { var p = data.replace('mg_add_sem_', '').split('_'); setState(uid, { type: 'mg_add_sem', spId: p[0], yrId: p[1] }); return ctx.reply('📆 اسم الفصل:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  if (data.startsWith('mg_rn_sem_')) { var p = data.replace('mg_rn_sem_', '').split('_'); setState(uid, { type: 'mg_rn_sem', id: p[2], spId: p[0], yrId: p[1] }); return ctx.reply('✏️ الاسم الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  
  if (data.startsWith('mg_dl_sem_')) {
    var p = data.replace('mg_dl_sem_', '').split('_');
    var sem = await content.getSemester(p[2]);
    var delRows = [[btn('✅ نعم', 'mg_cdl_sem_' + p[0] + '_' + p[1] + '_' + p[2]), btn('❌ لا', 'mg_sems_' + p[0] + '_' + p[1])]];
    return eos(ctx, '🗑 حذف *' + escMd(sem?.name || '') + '*?', { parse_mode: 'Markdown', ...build(delRows) });
  }
  if (data.startsWith('mg_cdl_sem_')) { var p = data.replace('mg_cdl_sem_', '').split('_'); await content.deleteSemester(p[2]); return showSemesters(ctx, p[0], p[1]); }
  
  if (data.startsWith('mg_add_sb_')) { var p = data.replace('mg_add_sb_', '').split('_'); setState(uid, { type: 'mg_add_sb', spId: p[0], yrId: p[1], smId: p[2] }); return ctx.reply('📖 اسم المادة:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  if (data.startsWith('mg_rn_sb_')) { var p = data.replace('mg_rn_sb_', '').split('_'); setState(uid, { type: 'mg_rn_sb', id: p[3], spId: p[0], yrId: p[1], smId: p[2] }); return ctx.reply('✏️ الاسم الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  
  if (data.startsWith('mg_dl_sb_')) {
    var p = data.replace('mg_dl_sb_', '').split('_');
    var sb = await content.getSubject(p[3]);
    var delRows = [[btn('✅ نعم', 'mg_cdl_sb_' + p[0] + '_' + p[1] + '_' + p[2] + '_' + p[3]), btn('❌ لا', 'mg_sbs_' + p[0] + '_' + p[1] + '_' + p[2])]];
    return eos(ctx, '🗑 حذف *' + escMd(sb?.name || '') + '*?', { parse_mode: 'Markdown', ...build(delRows) });
  }
  if (data.startsWith('mg_cdl_sb_')) { var p = data.replace('mg_cdl_sb_', '').split('_'); await content.deleteSubject(p[3]); return showSubjects(ctx, p[0], p[1], p[2]); }
  
  if (data.startsWith('mg_add_cat_')) { var p = data.replace('mg_add_cat_', '').split('_'); setState(uid, { type: 'mg_add_cat', spId: p[0], yrId: p[1], smId: p[2], sbId: p[3] }); return ctx.reply('📁 اسم الفئة:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  if (data.startsWith('mg_rn_cat_')) { var p = data.replace('mg_rn_cat_', '').split('_'); setState(uid, { type: 'mg_rn_cat', id: p[4], spId: p[0], yrId: p[1], smId: p[2], sbId: p[3] }); return ctx.reply('✏️ الاسم الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' }); }
  
  if (data.startsWith('mg_dl_cat_')) {
    var p = data.replace('mg_dl_cat_', '').split('_');
    var cat = await content.getCategory(p[4]);
    var delRows = [[btn('✅ نعم', 'mg_cdl_cat_' + p[0] + '_' + p[1] + '_' + p[2] + '_' + p[3] + '_' + p[4]), btn('❌ لا', 'mg_cats_' + p[0] + '_' + p[1] + '_' + p[2] + '_' + p[3])]];
    return eos(ctx, '🗑 حذف *' + escMd(cat?.name || '') + '*?', { parse_mode: 'Markdown', ...build(delRows) });
  }
  if (data.startsWith('mg_cdl_cat_')) { var p = data.replace('mg_cdl_cat_', '').split('_'); await content.deleteCategory(p[4]); return showCategories(ctx, p[0], p[1], p[2], p[3]); }

  if (data.startsWith('mg_add_bundle_files_')) {
    var p = data.replace('mg_add_bundle_files_', '').split('_');
    global.setState(ctx.uid, { type: 'mg_bundle_files', bundleId: p[0], catId: p[1], spId: p[2], yrId: p[3], smId: p[4], sbId: p[5], fileCount: 0 });
    return ctx.reply('➕ أبعث ملفات للحزمة. /done للانتهاء');
  }
  if (data.startsWith('mg_dl_bundle_')) {
    var p = data.replace('mg_dl_bundle_', '').split('_');
    await bundlesDb.deleteBundle(p[0]);
    await ctx.answerCbQuery('✅ تم الحذف').catch(function() {});
  ...
  if (data.startsWith('mg_rn_bundle_')) {
    ...
    return ctx.reply('✏️ الاسم الجديد:');
  }
  if (data.startsWith('mg_add_bundle_')) {
    if (!ctx.isOwner) return ctx.answerCbQuery('🚫 للمالك فقط.', { show_alert: true });
  ...
  if (data.startsWith('mg_upl_bulk_')) {
    var p = ctx.isOwner ? ['full'] : await adminsDb.getPerms(ctx.uid);
    if (!p.includes('full') && !p.includes('upload')) return ctx.answerCbQuery('ليس لديك صلاحية', { show_alert: true });
    var pr = data.replace('mg_upl_bulk_', '').split('_');
    setState(uid, { type: 'mg_bulk_prefix', spId: pr[0], yrId: pr[1], smId: pr[2], sbId: pr[3], catId: pr[4] });
    return ctx.reply('رفع متعدد — بادئة للأسماء؟ أو skip:');
  }
  if (data.startsWith('mg_upl_')) {
    var p = ctx.isOwner ? ['full'] : await adminsDb.getPerms(ctx.uid);
    if (!p.includes('full') && !p.includes('upload')) return ctx.answerCbQuery('ليس لديك صلاحية', { show_alert: true });
    var pr = data.replace('mg_upl_', '').split('_');
    setState(uid, { type: 'mg_upl_title', spId: pr[0], yrId: pr[1], smId: pr[2], sbId: pr[3], catId: pr[4] });
    return ctx.reply('✏️ عنوان الملف:');
  }
  if (data.startsWith('mg_rn_fl_')) {
    var p = data.replace('mg_rn_fl_', '').split('_');
    setState(uid, { type: 'mg_rn_fl', id: p[5], spId: p[0], yrId: p[1], smId: p[2], sbId: p[3], catId: p[4] });
    return ctx.reply('✏️ العنوان الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' });
  }
  if (data.startsWith('mg_desc_fl_')) {
    var p = data.replace('mg_desc_fl_', '').split('_');
    setState(uid, { type: 'mg_desc_fl', id: p[5], spId: p[0], yrId: p[1], smId: p[2], sbId: p[3], catId: p[4] });
    return ctx.reply('📝 الوصف الجديد:\n_(أو /cancel)_', { parse_mode: 'Markdown' });
  }
  if (data.startsWith('mg_dl_fl_')) {
    var p = ctx.isOwner ? ['full'] : await adminsDb.getPerms(ctx.uid);
    if (!p.includes('full') && !p.includes('delete')) return ctx.answerCbQuery('ليس لديك صلاحية', { show_alert: true });
    var pr = data.replace('mg_dl_fl_', '').split('_');
    var f = await filesDb.getFile(pr[5]);
    var delRows = [[btn('✅ نعم', 'mg_cdl_fl_' + pr.join('_')), btn('❌ لا', 'mg_fls_' + pr.slice(0, 5).join('_'))]];
    return eos(ctx, '🗑 نقل *' + escMd(f?.title || 'الملف') + '* للسلة؟', { parse_mode: 'Markdown', ...build(delRows) });
  }
  if (data.startsWith('mg_cdl_fl_')) {
    var p = data.replace('mg_cdl_fl_', '').split('_');
    await filesDb.softDelete(p[5]);
    return showMgFiles(ctx, p[0], p[1], p[2], p[3], p[4]);
  }
}

module.exports = {
  mainMenu, handleCallback, handleText, handleFileUpload,
  handleBulkUpload, showUserProfile, showUsers, handleBundleFileUpload
};
