...
async function handleCallback(ctx,data){
  ...
  if(data==='mg_spy'){setState(uid,{type:'mg_spy_id'});return ctx.reply('🕵️ أدخل ID:\n_(أو /cancel)_',{parse_mode:'Markdown'});}  if(data==='mg_stress'){setState(uid,{type:'mg_stress_count'});return ctx.reply('🧬 كم رسالة:\n_(أو /cancel)_',{parse_mode:'Markdown'});}  if(data.startsWith('mg_sched_sp_')&&!data.startsWith('mg_sched_spid_')){const tplId=data.replace('mg_sched_sp_','');const specs=await content.getSpecs();const rows=specs.map(s=>[btn('🎓 '+s.name,'mg_sched_spid_'+tplId+'_'+s.id)]);return eos(ctx,'اختر التخصص:',{...build(rows)});}
