...
async function mainMenu(ctx){
  ...
  if(isOwner(ctx.uid)){
    ...
    rows.push([btn('💾 نسخ احتياطي','mg_backup'),btn(store.maintenance?'🟢 إيقاف الصيانة':'🔴 وضع الصيانة','mg_maint')]);
...
async function handleText(ctx,state){
  ...
  try{
    switch(state.type){
      ...
      case 'mg_maint_msg':store.maintenanceMsg=text;clearState(uid);ctx.reply('✅ تم تحديث رسالة الصيانة',build([back('mg_menu')]));break;
...
async function handleCallback(ctx,data){
  ...
  if(data==='mg_maint'){store.maintenance=!store.maintenance;await setSetting('maintenance',store.maintenance?'true':'false');await interactions.addLog(uid,'maintenance',store.maintenance?'ON':'OFF');return eos(ctx,'🔧 *الصيانة: '+(store.maintenance?'🔴 مفعّلة':'🟢 متوقفة')+'*',{parse_mode:'Markdown',...build([[btn(store.maintenance?'🟢 إيقاف':'🔴 تفعيل','mg_maint')],[btn('📝 تعديل الرسالة','mg_set_maint_msg'),btn('◀️ رجوع','mg_menu')]])});}
