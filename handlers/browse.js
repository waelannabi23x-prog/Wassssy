...
async function showSpecs(ctx){var specs=cacheGet('specs_all');if(!specs){specs=await content.getSpecs();cacheSet('specs_all',specs,86400000);}var rows=specs.map(function(s){return[btn('🎓 '+s.name,'sp_'+s.id)];});rows.push(back('main_menu'));return eos(ctx,'🎓 *اختر تخصصك:*',{parse_mode:'Markdown',...build(rows)});}

async function showYears(ctx,spId,page){spId=safeInt(spId);page=safeInt(page);var ckey='yrs_'+spId;var yd=cacheGet(ckey);if(!yd){var results=await Promise.all([content.getSpec(spId),content.getYears(spId)]);yd={sp:results[0],all:results[1]};cacheSet(ckey,yd,86400000);}var sp=yd.sp,all=yd.all;var total=all.length;var years=all.slice(page*PS,(page+1)*PS);if(!years.length)return eos(ctx,buildPath([escMd(sp?sp.name:'')])+'\n\n📭 لا توجد سنوات.',build([backMenu('browse')]));var rows=years.map(function(y){return[btn('📅 '+y.name,'yr_'+spId+'_'+y.id)];});if(total>PS){var nav=[];if(page>0)nav.push(btn('⬅️','yr_page_'+spId+'_'+(page-1)));nav.push(btn((page+1)+'/'+Math.ceil(total/PS),'noop'));if((page+1)*PS<total)nav.push(btn('➡️','yr_page_'+spId+'_'+(page+1)));rows.push(nav);}rows.push(backMenu('browse'));// Prefetch محتوى السنوات في الخلفية
