...
async function doReport(ctx,fid,reason,spId,yrId,smId,sbId,catId){fid=safeInt(fid);var uid=ctx.uid;var already=await reportsDb.hasReported(uid,fid);if(already)return ctx.answerCbQuery('🚩 لقد أبلغت عن هذا الملف مسبقاً',{show_alert:true}).catch(function(){});await reportsDb.addReport(fid,uid,reason);await ctx.answerCbQuery('✅ تم إرسال التبليغ، شكراً!',{show_alert:true}).catch(function(){});return showPreview(ctx,fid,spId,yrId,smId,sbId,catId);}
