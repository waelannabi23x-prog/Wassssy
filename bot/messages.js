...
module.exports.registerMessages = function(bot, deps) {
  ...
  bot.on('message', async (ctx, next) => {
    ...
    if (ctx.chat?.type !== 'private' && ctx.from && !ctx.isAdmin && !ctx.isOwner) {
      ...
      if (grpSettings.anti_flood) {
        const sp = _spamMap.get(uid + '_' + cid) || { count: 0, last: now };
        if (now - sp.last < 3000) { sp.count++; sp.last = now; }
        else { sp.count = 1; sp.last = now; }
        _spamMap.set(uid + '_' + cid, sp);
        if (sp.count > 5) {
        ...
        if (sp.count > 8) { ctx.deleteMessage().catch(() => {}); return next(); }
      }
